...
import TransactionSaleBusiness from "./sale.business"
import {TransactionRepository} from "../transaction.repository";
import {Messages} from "../../../constants";
import * as Excel from 'exceljs'
import path from 'path'

export class TransactionSaleController extends BaseController<ITransactionSale> {
    constructor() {
        super(new TransactionSaleBusiness(), "transactionSale", true, new TransactionSaleRepository())
        this.init()
    }

    register(express: Application) {
        express.use('/api/v1/transaction/sale', guard, this.router);
    }
    ...
    async find(req: Request, res: Response): Promise<void> {
        ...
        await new TransactionSaleController().findBC(req, res, populate)
    }

    async index(req: Request, res: Response): Promise<void> {
        res.locals = {status: false, message: Messages.FETCH_FAILED};
        const {data, page}: any = await new TransactionSaleRepository().index(req.query as any)
    ...
    //     const skuIds = await new TransactionSaleRepository().getSkuIds(transactionId)
    //     await new TransactionSaleController().paginationOfAnArray(req, res, skuIds)
    // }

    async findReview(req: Request, res: Response): Promise<void> {
        res.locals = {status: false, message: Messages.FETCH_FAILED};
        //@ts-expect-error
        req.query.transactionData = await new TransactionSaleRepository().findOneBR({transactionId: req?.query?.transactionId});
    ...
    async filterReview(req: Request, res: Response): Promise<void> {
        res.locals = {status: false, message: Messages.FETCH_FAILED};
        let transactionData = await new TransactionSaleRepository().findOneBR({transactionId: req?.query?.transactionId});
        //@ts-expect-error
    ...
    async exportReport(req: Request, res: Response): Promise<void> {
        ...
        let worksheet = workbook.addWorksheet('TransactionSale Export')
        await new TransactionRepository().exportExcel(worksheet, headerData, requiredData)
        let fileName = 'TransactionSale-Export.xlsx'
    ...
    async filter(req: Request, res: Response): Promise<void> {
        ...
        let data = await new TransactionSaleRepository().aggregateBR(aggregateCond).then(data => data[0])
        console.log(data);
