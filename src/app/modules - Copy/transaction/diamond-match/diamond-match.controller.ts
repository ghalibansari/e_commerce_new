...
import TransactionDmBusiness from "./diamond-match.business"
...
export class TransactionDmController extends BaseController<ITransactionDm> {
    constructor() {
        super(new TransactionDmBusiness(), "transactionDm", true)
        this.init()
    }

    register(express: Application) {
        express.use('/api/v1/transaction/diamond-match', guard, this.router);
    }
    ...
    async find(req: Request, res: Response): Promise<void> {
        ...
        await new TransactionDmController().findBC(req, res, populate)
    }

    async index(req: Request, res: Response): Promise<void> {
        res.locals = {status: false, message: Messages.FETCH_FAILED};
        const {data, page}: any = await new TransactionDmRepository().index(req.query as any)
    ...
    //     let skuIds = await new TransactionDmRepository().getSkuIds(transactionId)
    //     await new TransactionDmController().paginationOfAnArray(req, res, skuIds)
    // }

    async findReview(req: Request, res: Response): Promise<void> {
        res.locals = {status: false, message: Messages.FETCH_FAILED};
        //@ts-expect-error
        req.query.transactionData = await new TransactionDmRepository().findOneBR({transactionId: req?.query?.transactionId});
    ...
    async filterReview(req: Request, res: Response): Promise<void> {
        res.locals = {status: false, message: Messages.FETCH_FAILED};
        let transactionData = await new TransactionDmRepository().findOneBR({transactionId: req?.query?.transactionId});
        //@ts-expect-error
    ...
    async exportReport(req: Request, res: Response): Promise<void> {
        ...
        let worksheet = workbook.addWorksheet('TransactionDM Export')
        await new TransactionRepository().exportExcel(worksheet, headerData, requiredData)
        let fileName = 'TransactionDiamondMatch-Export.xlsx'
    ...
    async filter(req: Request, res: Response): Promise<void> {
        ...
        let data = await new TransactionDmRepository().aggregateBR(aggregateCond).then(data => data[0])
        console.log(data);
