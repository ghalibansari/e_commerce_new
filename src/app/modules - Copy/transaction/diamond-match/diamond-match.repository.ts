...
import transactionDmModel from "./diamond-match.model";
import { ITransactionDm } from "./diamond-match.types";
import skuModel from "../../sku/sku.model";
import { ISku } from "../../sku/sku.types";
import mongoose from "mongoose";
import {ICond, IIndexParam} from "../../../interfaces/IRepository";
import {IUserNested} from "../../user/user.types";


export class TransactionDmRepository extends BaseRepository<ITransactionDm> {
    constructor(){
        super(transactionDmModel)
    }
    ...
    async getSkuIds( transactionId: any): Promise<ISku[]> {
        let transactionData: any = await transactionDmModel.findOne({ transactionId: transactionId });
