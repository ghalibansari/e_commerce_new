...
import TransactionConsignmentBusiness from "./consignment.business"
...
export class TransactionConsignmentController extends BaseController<ITransactionConsignment> {
    constructor() {
        super(new TransactionConsignmentBusiness(), "transactionConsignment", true)
        this.init()
    }

    register(express: Application) {
        express.use('/api/v1/transaction/consignment', guard, this.router);
    }
    ...
    async find(req: Request, res: Response): Promise<void> {
        ...
        await new TransactionConsignmentController().findBC(req, res, populate)
    }

    async index(req: Request, res: Response): Promise<void> {
        res.locals = {status: false, message: Messages.FETCH_FAILED};
        const {data, page}: any = await new TransactionConsignmentRepository().index(req.query as any)
    ...
    //     let skuIds = await new TransactionConsignmentRepository().getSkuIds(transactionId)
    //     await new TransactionConsignmentController().paginationOfAnArray(req, res, skuIds)
    // }

    async findReview(req: Request, res: Response): Promise<void> {
        res.locals = {status: false, message: Messages.FETCH_FAILED};
        //@ts-expect-error
        req.query.transactionData = await new TransactionConsignmentRepository().findOneBR({transactionId: req?.query?.transactionId});
    ...
    async filterReview(req: Request, res: Response): Promise<void> {
        res.locals = {status: false, message: Messages.FETCH_FAILED};
        let transactionData = await new TransactionConsignmentRepository().findOneBR({transactionId: req?.query?.transactionId});
        //@ts-expect-error
    ...
    async exportReport(req: Request, res: Response): Promise<void> {
        ...
        let worksheet = workbook.addWorksheet('TransactionConsignment Export')
        await new TransactionRepository().exportExcel(worksheet, headerData, requiredData)
        let fileName = 'TransactionConsignment-Export.xlsx'
    ...
    async filter(req: Request, res: Response): Promise<void> {
        ...
        let user = await new UserRepository().findOneBR({ _id : loggedInUserId },{},[{ path: 'roleId' }])
        let cond: any = {}
        console.log(user);        
        //@ts-expect-error
        ...
        let data = await new TransactionConsignmentRepository().aggregateBR(aggregateCond).then(data => data[0])
        res.locals = { status: true, message: Messages.FETCH_SUCCESSFUL, data };
        await JsonResponse.jsonSuccess(req, res, `{this.url}.findReview`);
    }
