...
import TransactionIavChangeBusiness from "./iav-change.business"
import {Messages} from "../../../constants";
import {TransactionRepository} from "../transaction.repository";
import * as Excel from 'exceljs'
import path from 'path'

export class TransactionIavChangeController extends BaseController<ITransactionIav> {
    constructor() {
        super(new TransactionIavChangeBusiness(), "transactionImport", true)
        this.init()
    }

    register(express: Application) {
        express.use('/api/v1/transaction/iav-change', guard, this.router);
    }
    ...
    async find(req: Request, res: Response): Promise<void> {
        ...
        await new TransactionIavChangeController().findBC(req, res, populate)
    }

    async index(req: Request, res: Response): Promise<void> {
        res.locals = {status: false, message: Messages.FETCH_FAILED};
        const {data, page}: any = await new TransactionIavChangeRepository().index(req.query as any)
    ...
    //     let skuIds = await new TransactionIavChangeRepository().getSkuIds(transactionId)
    //     await new TransactionIavChangeController().paginationOfAnArray(req, res, skuIds)
    // }

    async findReview(req: Request, res: Response): Promise<void> {
        res.locals = {status: false, message: Messages.FETCH_FAILED};
        //@ts-expect-error
        req.query.transactionData = await new TransactionIavChangeRepository().findOneBR({transactionId: req?.query?.transactionId});
    ...
    async filterReview(req: Request, res: Response): Promise<void> {
        res.locals = {status: false, message: Messages.FETCH_FAILED};
        let transactionData = await new TransactionIavChangeRepository().findOneBR({transactionId: req?.query?.transactionId});
        //@ts-expect-error
    ...
    async exportReport(req: Request, res: Response): Promise<void> {
        ...
        let worksheet = workbook.addWorksheet('TransactionIav Export')
        await new TransactionRepository().exportExcel(worksheet, headerData, requiredData)
        let fileName = 'TransactionIav-Export.xlsx'
    ...
    async filter(req: Request, res: Response): Promise<void> {
        ...
        let data = await new TransactionIavChangeRepository().aggregateBR(aggregateCond).then(data => data[0])
        console.log(data);
