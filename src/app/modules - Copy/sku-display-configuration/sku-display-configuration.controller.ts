...
import SkuDisplayConfigurationBusiness from "./sku-display-configuration.business";
import {SkuDisplayConfigurationRepository} from "./sku-display-configuration.repository";
import {SkuDisplayConfigurationValidation} from "./sku-display-configuration.validation";
import {ISkuDisplayConfigurationTypes} from "./sku-display-configuration.types";


//Todo delete module...

export class SkuDisplayConfigurationController extends BaseController<ISkuDisplayConfigurationTypes> {
    constructor() {
        super(new SkuDisplayConfigurationBusiness(), "activity-display-configuration", true, new SkuDisplayConfigurationRepository());
        this.init();
    }

    register(express: Application) {
        express.use('/api/v1/sku-display-configuration', guard, this.router);
    }

    init() {
        const validation: SkuDisplayConfigurationValidation = new SkuDisplayConfigurationValidation();
    ...
    async find(req: Request, res: Response){
        ...
        await new SkuDisplayConfigurationController().findBC(req, res)
    }

    async findById(req: Request, res: Response){
        let populate = [{path: 'companyId'}, {path: 'roleId'}];
        await new SkuDisplayConfigurationController().findByIdBC(req, res, populate)
    }
