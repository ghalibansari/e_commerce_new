...
import alertSubCategoryModel from "./alert-sub-category.model";
import alertCategoryModel from "../alert-category/alert-category.model";
import { IUser } from "app/modules/user/user.types";

export class AlertSubCategoryRepository extends BaseRepository<IAlertSubCategory> {
    constructor() {
        super(alertSubCategoryModel);
    }
    ...
    index = async ({ filters, search, sort: sorter, pageNumber, pageSize, column }: IIndexParam): Promise<ILoggerNested[]> => {
        ...
        if (search) {
            ...
            secondCond['$or'] = [{ 'subCategory': _S }, { 'alertCategoryId.category': _S }]
        }
        ...
        const aggregate = [
            { $lookup: { from: alertCategoryModel.collection.name, as: 'alertCategoryId', let: {id: '$alertCategoryId'}, pipeline: [{$match: {$expr: {$eq: ['$_id', '$$id']}}}, {$match: {isDeleted: false }}] } },
            { $unwind: { path: "$alertCategoryId", preserveNullAndEmptyArrays: true } },
            { $lookup: { from: userModel.collection.name, localField: 'createdBy', foreignField: '_id', as: 'createdBy' } },// Todo remove all normal lookup and use pipeline and isDeleted.
    ...
    create = async (data:any) => {
        const duplicate = await alertSubCategoryModel.findOne({isDeleted: false, alertCategoryId: data.alertCategoryId, subCategory: data.subCategory}).select('_id').lean()
        if(duplicate?._id) throw 'Duplicate record, Record already present'
        return await alertSubCategoryModel.create(data)
    }

    async filter(userId: IUser['_id']): Promise<any> {
        return await alertSubCategoryModel.aggregate([
            { $match: { isDeleted: false } },
            { $lookup: { from: alertCategoryModel.collection.name, as: 'alertCategoryId', let: {id: '$alertCategoryId'}, pipeline: [{$match: {$expr: {$eq: ['$_id', '$$id']}}}, {$match: {isDeleted: false }}] } },
            { $unwind: { path: "$alertCategoryId", preserveNullAndEmptyArrays: true } },
            { $group: { _id: null, 'category': { "$addToSet": "$alertCategoryId.category" },  subCategory: { $addToSet: '$subCategory' } }},
            { $project: { _id: 0, 'category': 1, subCategory: 1 } }
        ]).then(([data]) => data)
    }
}