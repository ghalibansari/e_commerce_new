...
> {
  ...
  register = (express: Application) =>
    express.use(`/api/v1/${this.url}`, AuthGuard, this.router);

  init() {
    this.router.get("/", TryCatch.tryCatchGlobe(this.index));
    this.router.get(
      "/:id",
      validateParams(UserAddressValidation.findById),
      TryCatch.tryCatchGlobe(this.findByIdBC)
    );
    this.router.post(
      "/",
      validateBody(UserAddressValidation.addUserAddress),
      DBTransaction.startTransaction,
      TryCatch.tryCatchGlobe(this.addUserAddress)
    );
    // this.router.post("/bulk", validateBody(UserAddressValidation.addUserAddressBulk), TryCatch.tryCatchGlobe(this.createBulkBC))
    this.router.put(
      "/:id",
      validateBody(UserAddressValidation.updateUserAddress),
      DBTransaction.startTransaction,
      TryCatch.tryCatchGlobe(this.updateUserAddress)
    );
  ...
  addUserAddress = async (req: Request, res: Response): Promise<void> => {
    ...
    const userAdd = await this.repo.findBulkBR({
      where: { user_id },
    ...
    if (!userAdd.length) body.is_default = true;
      ...
      await this.repo.updateBulkBR({
        //@ts-expect-error
        where: { address_id: emptyArray },
        newData: { is_default: false },
        updated_by: user_id,
        transaction,
      });
    }
    await this.repo.createOneBR({
      newData: { ...body, user_id },
      created_by: user_id,
      transaction,
    });
  ...
  updateUserAddress = async (req: Request, res: Response): Promise<void> => {
    ...
    const address = await this.repo.findBulkBR({
      where: { user_id },
      attributes: ["address_id", "is_default"],
    });
    ...
    if (address.length == 1 && address[0].address_id == id)
    ...
    const { count } = await this.repo.updateByIdBR({
      id,
      newData: body,
      updated_by: user_id,
    });
