...
import { IMBrand } from "../brand/brand.types";
import { CategoriesRepository } from "../categories/categories.repository";
import { IMCategories } from "../categories/categories.type";
import { ProductRepository } from "../products/product.repository";
import { IMProduct } from "../products/product.type";
...
export class CustomRepository {
    ...
    home = async (): Promise<any> => {
        ...
        const [brandHeader, categoriesHeader, banner, categories, subCategories, brand] = await Promise.all([
            ...
            await CategoriesRepo.findBulkBR({ where: { show_on_header: true, parent_ids: null }, include: [{ model: CategoriesRepo._model, as: 'sub_cat', attributes: ['category_name', 'category_id'] }], attributes: ['category_name', 'category_id'] }),
    ...
    filter = async (): Promise<{ categories: [] | IMCategories[]; amountMinmax: IMProduct[]; brand: [] | IMBrand[]; }> => {
