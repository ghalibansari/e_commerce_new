...
export class CustomRepository {
    ...
    home = async (): Promise<any> => {
        ...
        const [brandHeader, categoriesHeader, banner, categories, subCategories, brand] = await Promise.all([
            ...
            await CategoriesRepo.findBulkBR({ where: { show_on_header: true, parent_id: null }, include: [{ model: CategoriesRepo._model, as: 'sub_cat', attributes: ['category_name', 'category_id'], include: [{ model: CategoriesRepo._model, as: 'sub_cat', attributes: ['category_name', 'category_id'] }] }], raw: false, attributes: ['category_name', 'category_id'] }),

            await new BannerRepository().findBulkBR({ where: { show_on_home_screen: true } }),
            await CategoriesRepo.findBulkBR({ where: { show_on_home_screen: true, parent_id: null }, include: [{ model: CategoriesRepo._model, as: 'sub_cat', attributes: ['category_name', 'category_id'] }], raw: false, attributes: ['category_name', 'category_id'] }),
    ...
    filter = async (): Promise<any> => {
        ...
        const [categories, brand, amountMinmax] = await Promise.all([
            await CategoriesRepo.findBulkBR({ where: { parent_id: null }, include: [{ model: CategoriesRepo._model, as: 'sub_cat', attributes: ['category_name', 'category_id'], include: [{ model: CategoriesRepo._model, as: 'sub_cat', attributes: ['category_name', 'category_id'] }] }], raw: false, attributes: ['category_name', 'category_id'] }),
