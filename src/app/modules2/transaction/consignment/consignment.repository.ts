...
import transactionConsignmentModel from "./consignment.model";
...
export class TransactionConsignmentRepository extends BaseRepository<ITransactionConsignment> {
    constructor(){
        super(transactionConsignmentModel)
    }

    index = async ({filters, search, sort:sorter, pageNumber, pageSize, column}: IIndexParam): Promise<ITransactionConsignment[]> => {
        //@ts-expect-error
        let cond: ICond = {'isDeleted': false}, sort = {}, projection: IIndexProjection = {'createdBy.password': 0, 'updatedBy.password': 0}
        ...
        if(column && column[0]=='[' && column[column.length-1]==']') {
            ...
            // projection['createdBy.password'] = 0
            // projection['updatedBy.password'] = 0
        }
    ...
    async getSkuIds( transactionId: any): Promise<ISku[]> {
        let transactionData: any = await transactionConsignmentModel.findOne({ transactionId: transactionId });
