...
export class DeviceTypeRepository extends BaseRepository<IDeviceType> {
    constructor () {
        super(deviceTypeModel);
    }
    ...
    delete = async (deviceTypeId: IDeviceType['_id'], user: IUser['_id']): Promise<any> => {
        let [deviceType, device] = await Promise.all([
            await deviceTypeModel.findById({_id: deviceTypeId, isDeleted: false}),
            await deviceModel.find({deviceTypeId, isDeleted: false})
        ]);
        if(device.length > 0) throw Errors.DEVICETYPEID_LINKED_WITH_ACTIVE_DEVICE
        else if(!deviceType) throw Errors.INVALID_ID
        else await deviceTypeModel.findOneAndUpdate({_id: deviceTypeId}, {isDeleted: true, updatedBy: user})
    }
    ...
    async filter(userId: IUser['_id']): Promise<any> {
        ...
        let data = await deviceTypeModel.aggregate([   //Todo remove all sting in from fields in lookup joints.
