const app = angular.module("categoriesApp", [
    "ngSanitize",
    "ngMessages",
    "ui.bootstrap",
    "ui.bootstrap.modal",
  ...
  app.controller("categoriesAppCtrlr", [
    "$scope",
    "$http",
    "$window",
    "$uibModal",
    ...
    (
      ...
      function actionsHtml(data, type, full, meta) {
        ...
        return `<button class="btn btn-warning" ng-click='onOpenModalCick("edit", ${obj}, ${meta.row})' > <i class="fa fa-edit text-white"></i> </button> 
            <button class="btn btn-danger" ng-click='delete("${full.name}", "${full.category_id}")' > <i class="fa fa-trash text-white"></i> </button> `;
      }
  ...
  app.controller("categoryDetails", [
    "$scope",
    "$http",
    "record",
    "$window",
    ($scope, $http, record, $window) => {
      ...
      $scope.update = () => {
        $scope.isPosting = true;
        $http
          .put(`${$scope.uri}/${$scope.category.category_id}`, $scope.category)
          .then((res) => {
            Swal.fire({
              title: "Success",
